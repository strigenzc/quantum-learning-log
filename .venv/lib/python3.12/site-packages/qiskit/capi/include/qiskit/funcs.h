 ...
 */
void qk_quantum_register_circuit_bits(const QkQuantumRegister *qreg,
 ...
 */
void qk_classical_register_circuit_bits(const QkClassicalRegister *creg,
                                        const QkCircuit *circuit,
                                        uint32_t *out_bits);
 ...
 */
const QkClassicalRegister *qk_circuit_get_classical_register(const QkCircuit *circuit,
                                                             size_t creg_idx);
 ...
 */
QkControlFlowInstruction *qk_circuit_get_control_flow_instruction(const QkCircuit *circuit,
                                                                  size_t inst_idx,
                                                                  const QkControlFlowInstruction *parent_cf);
 ...
 * Behavior is undefined if ``cf_inst`` is not a valid pointer to a ``QkControlFlowInstruction``.
 */
const QkCircuit *qk_control_flow_block_circuit(const QkControlFlowInstruction *cf_inst,
                                               size_t block_idx);
 ...
 */
void qk_control_flow_loop_range(const QkControlFlowInstruction *cf_inst,
                                int64_t *out_start,
                                int64_t *out_stop,
                                int64_t *out_step);
 ...
 * Behavior is undefined if ``cf_inst`` is not a valid pointer to a ``QkControlFlowInstruction``.
 */
bool qk_control_flow_switch_is_case_default(const QkControlFlowInstruction *cf_inst,
                                            size_t case_idx);
 ...
 * Behavior is undefined if ``cf_inst`` is not a valid pointer to a ``QkControlFlowInstruction``.
 */
uint64_t qk_control_flow_switch_case_labels_bit_width(const QkControlFlowInstruction *cf_inst,
                                                      size_t case_idx);
 ...
 */
QkSwitchCaseLabels qk_control_flow_switch_case_labels_uint(const QkControlFlowInstruction *cf_inst,
                                                           size_t case_idx);
 ...
 */
void qk_transpiler_pass_standalone_2q_peephole_optimization(QkCircuit *circuit,
                                                            const QkTarget *target,
                                                            double approximation_degree);
 ...
 * class to run this pass if there are circuit elements owned by Python.
 */
void qk_transpiler_pass_2q_peephole_optimization(QkDag *dag,
                                                 const QkTarget *target,
                                                 double approximation_degree);
